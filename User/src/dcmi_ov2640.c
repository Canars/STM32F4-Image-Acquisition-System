/**
  ...
  * @version V1.0
  * @date    27-March-2014
  * @brief   This file includes the driver for OV2640 Camera module
  ******************************************************************************
*/
...
u8 Dma_FreeBuf_No=0;//DMA空闲缓冲区号标志
u8 Dma_FreeBuf_Ok=0;//DMA缓冲区空闲标志
...
const char OV2640_QQVGA[][2]=
{
  ...
  0xff, 0x01,
  0x3c, 0x32,
  0x11, 0x00,////////////////////////0x11,0xD3调整分频系数
  0x09, 0x02,
  0x03, 0xcf,//VEND 2 LSBs:11 ,VSTART 2 LSBs :11
  0x04, 0x08,
  0x13, 0xe5,
  0x14, 0x48,
  0x2c, 0x0c,
  ...
  0xd7, 0x03,
  0xd9, 0x10,
  0xd3, 0x82,//R_DVP_SP:Auto mode ,DVP PCLK = sysclk(48)/[6:0](YUV0)  | DVP PCLK = sysclk(48)/(2*[6:0])(RAW)
  0xc8, 0x08,
  0xc9, 0x80,
  0x7c, 0x00,
  0x7d, 0x02,
  ...
  0x19, 0x01,
  0x1a, 0x97,
  0x32, 0x36,//HREFEND 3 LSBs:110 ,HREFST 3 LSBs:110
  0x4f, 0xbb,
  0x6d, 0x80,
  0x3d, 0x34,
  0x39, 0x02,
  ...
  0x5c, 0x05,
  0xe0, 0x00,
  0xd3, 0x04,//R_DVP_SP:Auto mode ,DVP PCLK = sysclk(48)/[6:0](YUV0)  | DVP PCLK = sysclk(48)/(2*[6:0])(RAW)
  0xff, 0x00,
  0xc3, 0xef,
  0xa6, 0x00,
  0xa7, 0xdd,
  ...
  0x12, 0x40,
  0x17, 0x11,//137
  0x18, 0x43,//537 Horizontal
  0x19, 0x00,//3
  0x1a, 0x4b,//303 vertical 
  0x32, 0x09,//
  0x4f, 0xca,
  0x50, 0xa8,
  0x5a, 0x23,
  ...
  0xc1, 0x4b,
  0x86, 0x35,
  0x50, 0x92,//V divider 2,H divider 2 
  0x51, 0xc8,//Hsize :200*4 = 800
  0x52, 0x96,//Vsize:150*4=600
  0x53, 0x00,
  0x54, 0x00,
  0x55, 0x00,
  0x57, 0x00,
  0x5a, 0x28,//OUTW=40*4 = 160 0x28 
  0x5b, 0x1e,//OUTH=30*4 = 120
  0x5c, 0x00,
  0xe0, 0x00,
  0xff, 0x01,
  0x11, 0x00,
...
{
  0xff, 0x00,//register bank select :DSP address
  0x2c, 0xff,
  0x2e, 0xdf,
  0xff, 0x01,
  0x3c, 0x32,
  0x11, 0x30,//Clock rate control : CLK = XVCLK / (decimal vlaue of CLKRC[5:0]+1)
  0x09, 0x02,
  0x04, 0x28,
  0x13, 0xe5,
  0x14, 0x48,
  ...
  0x71, 0x94,
  0x73, 0xc1,
  0x12, 0x40, //0x40  COM7 : SVGA mode ,color bat test pattern off
  0x17, 0x11, //HREFST ,0x089=137, Horizontal window start MSB 8bits (3 LSBs in REG32[2:0](0x32))
  0x18, 0x43, //HREFEND,0x219=537, Horizontal window end MSB 8bits (3 LSBs in REG32[5:3](0x32))
  0x19, 0x00, //VSTRT,0x03=3, Vertical Line Start MSB 8bits(2 LSBs in COM1[1:0](0x03))
  0x1a, 0x4b, //VEND, 0x12f=303,Vertical Line Start MSB 8bits(2 LSBs in COM1[3:2](0x03))
  0x32, 0x09, //REG32, No effect on PCLK
  0x37, 0xc0,
  0x4f, 0x60,
  0x50, 0xa8,
  0x6d, 0x00,
  ...
  0xb2, 0x0f,
  0xc4, 0x5c,
  0xc0, 0x64,//HSIZE8[7:0]:Image Horizontal Size HSIZE[10:3]   0x320=800
  0xc1, 0x4B,//VSIZE8[7:0]:Image vertical size VSIZE[10:3]     0x258=600
  0x8c, 0x00,//SIZEL[5:0]: HSIZE[11],HSIZE[2:0],VSIZE[2:0]
  0x86, 0x3D,
  0x50, 0x00,
  0x51, 0xC8,//HSIZE[7:0]:real/4 , real = 200 * 4 = 800
  0x52, 0x96,//VSIZE[7:0]:real/4 , real = 150 * 4 = 600
  0x53, 0x00,//XOFFL[7:0]:OFFSET_X[7:0]
  0x54, 0x00,//YOFFL[7:0]:OFFSET_Y[7:0]
  0x55, 0x00,//VHYX[7:0],Bit[7]:VSIZE[8],Bit[6:4]:OFFSET_Y[10:8],Bit[3]:HSIZE[8],Bit[2:0]:OFFSET_X[10:8]
  0x5a, 0xC8,//output width : 200 * 4 = 800
  0x5b, 0x96,//output height: 150 * 4 = 600
  0x5c, 0x00,
  0xd3, 0x7f,//R_DVP_SP:Auto mode ,DVP PCLK = sysclk(48)/[6:0](YUV0)  | DVP PCLK = sysclk(48)/(2*[6:0])(RAW)
  0xc3, 0xed,
  0x7f, 0x00,
  0xda, 0x00,
  0xe5, 0x1f,
...
const unsigned char OV2640_JPEG[][2]=
{
  ...
  0xe5, 0x1f,
  0xd7, 0x03,
  0xda, 0x10,//JPEG output,YUV422,HREF is same as sensor,YUYV
  0xe0, 0x00,
  0xFF, 0x01,
  0x04, 0x08,
};
...
/* JPG 320x480 */
const unsigned char OV2640_320x480_JPEG[][2]=
{
  0xff, 0x01,
  0x12, 0x40,
  0x17, 0x11,
  ...
  0x55, 0x00,
  0x57, 0x00,
  0x5a, 0x50,//out width :  80 * 4 = 320
  0x5b, 0x78,//out height: 120 * 4 = 480
  0x5c, 0x00,
  0xe0, 0x00,
};
...
void OV2640_HW_Init(void)
{
  ...
  /* DCMI GPIO configuration */
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6 | GPIO_Pin_7; //DCMI_D0 DCMI_D1
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_4| GPIO_Pin_5| GPIO_Pin_6; //DCMI_D2 DCMI_D3 DCMI_D4 DCMI_D6 DCMI_D7
  GPIO_Init(GPIOE, &GPIO_InitStructure);   
    
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6 | GPIO_Pin_7; //DCMI_D5 DCMI_VSYNC 
...
/**
  * @brief  Configures DCMI/DMA to capture jpeg image from the OV2640 camera.
  * @param  ImageFormat: Image format BMP or JPEG 
  * @param  BMPImageSize: BMP Image size
  * @retval None
  */
void OV2640_JpegDcmiInit(void)
{
	...
	DMA_InitStructure.DMA_Channel = DMA_Channel_1;  
	DMA_InitStructure.DMA_PeripheralBaseAddr = DCMI_DR_ADDRESS;	
	DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)Image_Buffer1;
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;
...
void OV2640_JPEGConfig(ImageFormat_TypeDef ImageFormat)
{
  ...
  switch(ImageFormat)
  {
    ...
    case JPEG_320x480:
    {
      for(i=0; i<(sizeof(OV2640_320x480_JPEG)/2); i++)
      {
        OV2640_WriteReg(OV2640_320x480_JPEG[i][0], OV2640_320x480_JPEG[i][1]);
      }
      break;
    }
    default:
